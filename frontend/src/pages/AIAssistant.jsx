...
const AIAssistant = () => {
  ...
  const [voiceInputActive, setVoiceInputActive] = useState(false); // NEW: visual mode & speech suppress
  ...
  useEffect(() => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognition.onresult = (event) => {
        ...
        setVoiceInputActive(false); // turn off visual mode when done
      };
  ...
  const toggleListening = async () => {
    ...
    try {
      ...
      setIsListening(true);
      setVoiceInputActive(true); // activate red mode & suppress speech
    } catch (err) {
  ...
  const speak = (text) => {
    // Do NOT speak if voice input mode is active (red mic mode)
    if (!voiceEnabled || voiceInputActive) return;
    stopSpeaking();
  ...
  const makeApiCall = async (endpoint, data) => {
    ...
    try {
      const response = await api.post(endpoint, data, { signal: abortControllerRef.current.signal });
      return response;
    } catch (error) {
  ...
  const handleToolClick = async (tool) => {
    ...
    try {
      ...
      if (reply && reply !== "Request cancelled.") addAssistantMessage(reply);
    } catch (err) {
  ...
  return (
    <div className="h-[calc(100vh-64px)] flex overflow-hidden">
      {/* Sidebar (unchanged) */}
      <div className={`fixed inset-y-0 left-0 z-40 w-72 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-bold text-zinc-900 dark:text-white">AI Tools</h2>
            <button onClick={() => setSidebarOpen(false)} className="md:hidden p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800"><X className="w-5 h-5" /></button>
          </div>
          <div className="space-y-3">
            {tools.map(tool => (
              <button key={tool.name} onClick={() => handleToolClick(tool)} disabled={isLoading}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="md:hidden p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800"><Menu className="w-5 h-5" /></button>
            <div>
              <h1 className="text-xl font-extrabold text-zinc-900 dark:text-white flex items-center gap-2">
                <BrainCircuit className="w-6 h-6 text-indigo-600" /> AI Career Assistant
              </h1>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">Get personalized guidance for internships in Ethiopia</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={stopSpeaking} className="p-2 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all" title="Stop speaking">
              <Square className="w-5 h-5" />
            </button>
            <button onClick={() => setVoiceEnabled(!voiceEnabled)} className="p-2 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all" title={voiceEnabled ? "Disable voice replies" : "Enable voice replies"}>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-8">
              <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/20 rounded-3xl flex items-center justify-center border border-indigo-100 dark:border-indigo-800"><Bot className="w-10 h-10 text-indigo-600" /></div>
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-zinc-900 dark:text-white">Ready to help you grow</h3>
                <p className="text-zinc-500 dark:text-zinc-400">Choose a tool from the sidebar or ask me anything about internships and career guidance in Ethiopia!</p>
              </div>
            </div>
          ) : (
            messages.map((msg, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-4 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border ${msg.role === "user" ? "bg-zinc-100 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700" : "bg-indigo-600/10 border-indigo-500/20"}`}>
                  {msg.role === "user" ? <User className="w-5 h-5 text-zinc-500" /> : <Bot className="w-5 h-5 text-indigo-600" />}
                </div>
                <div className={`max-w-[80%] px-5 py-4 rounded-2xl leading-relaxed text-[15px] ${msg.role === "user" ? "bg-indigo-600 text-white" : "bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200"}`}>
          ...
          {isLoading && (
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-xl bg-indigo-600/10 border border-indigo-500/20 flex items-center justify-center"><Loader2 className="w-5 h-5 text-indigo-600 animate-spin" /></div>
              <div className="bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800 px-5 py-4 rounded-2xl">
                <div className="flex gap-1"><div className="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-bounce" /><div className="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-bounce [animation-delay:0.2s]" /><div className="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-bounce [animation-delay:0.4s]" /></div>
              </div>
            </div>
          )}
        </div>
          ...
          <form onSubmit={handleSend} className="relative flex gap-2">
            <div className="relative flex-1">
              <input
                ...
                placeholder={voiceInputActive ? "🎤 Listening... Speak now" : "Ask me anything about your career..."}
                className={`w-full bg-white dark:bg-zinc-800 border rounded-2xl py-4 pl-6 pr-14 focus:outline-none focus:ring-4 transition-all text-zinc-900 dark:text-white shadow-sm ${
                  voiceInputActive
                    ? "border-red-500 ring-red-200 dark:ring-red-900/30 bg-red-50 dark:bg-red-950/20 placeholder-red-500"
                    : "border-zinc-200 dark:border-zinc-700 focus:ring-indigo-500/10 focus:border-indigo-500"
                }`}
              />
              <button
                type="submit"
